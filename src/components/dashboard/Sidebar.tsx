...
import { LayoutDashboard, MessageSquareText, Package, Settings, Sparkles, Box, Zap, LogOut, Sliders, BarChart, FileText } from 'lucide-react';
...
const navItems = [
  ...
  { name: 'Inventory', href: '/dashboard/inventory', icon: Box, id: 'nav-inventory' },
...
export default function Sidebar() {
  ...
  // Check localStorage on mount — tour may already be permanently dismissed
  useEffect(() => {
    const dismissed = localStorage.getItem('dullbot_tour_dismissed');
    setIsTourDismissed(dismissed === '1');
  }, []);
    ...
    <>
      <aside className={`bg-fog border-r border-dove/20 hidden md:flex md:flex-col shrink-0 transition-all duration-300 relative z-30 ${isCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        <nav className="p-4 flex-1 space-y-1.5 overflow-y-auto overflow-x-hidden">
          ...
          <button
            ...
            title={isCollapsed ? "Business Context" : undefined}
            className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-inputs text-sm font-medium transition-colors text-ink bg-white border border-dove/10 hover:border-ink hover:shadow-subtle mb-4 ${isCollapsed ? 'justify-center px-2' : ''}`}
          >
            <FileText className="w-4 h-4 shrink-0 text-rust" />
            {!isCollapsed && <span className="truncate">Business Context</span>}
          </button>
      ...
      {shop && (
        <ContextModal 
          ...
          onSaveSuccess={(updatedShop) => setShop(updatedShop)}
        />
      )}
      
      {shop && !isTourDismissed && (
        <TourOverlay
          shop={shop}
          onDismiss={() => setIsTourDismissed(true)}
