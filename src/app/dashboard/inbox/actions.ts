...
import { getConversations as fetchConversations, getMessages as fetchMessages } from '@/lib/inbox-data';

export async function getMessages(conversationId: string, before?: string, limit: number = 40) {
  return fetchMessages(conversationId, before, limit);
}
...
export async function getConversations(shopId: string) {
  return fetchConversations(shopId);
}
