...
export default function OverviewClient({ shop: initialShop, productCount }: { shop: any; productCount: number }) {
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8 relative">
      ...
      {/* Onboarding Banner & Progress checklist */}
      {!shop.onboarding_complete && (
        <div className="mb-10 p-6 bg-fog rounded-cards border border-dove/25 shadow-subtle">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
            <div>
              ...
              <h1 className="text-3xl font-serif text-ink tracking-tight">Complete Launch Checklist</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {checklistItems.map((item, idx) => {
              ...
              const isSpotlight = completedStepsCount === idx; // Spotlight visual help on the next uncompleted step
                ...
                <div 
                  ...
                  className={`bg-white rounded-xl border p-4 transition-all duration-300 ${
                    item.isDone 
                      ...
                      : isSpotlight 
                        ? 'border-rust ring-1 ring-rust/30 shadow-subtle' 
                        : 'border-dove/20'
                  }`}
                >
                    ...
                    <div className="mt-4 pt-4 border-t border-dove/10 flex flex-col gap-4">
                      ...
                      {paymentMethod === 'notification_app' && (
                        <div className="p-4 bg-fog rounded-xl border border-dove/10 flex flex-col items-center gap-3">
                          ...
                          {/* Self-contained pixel-grid SVG QR code representation */}
      ...
      {/* Main Dashboard metrics (blurred if not complete) */}
      <div className={`transition-all duration-700 ${!shop.onboarding_complete ? 'opacity-35 select-none pointer-events-none blur-[2px]' : ''}`}>
        
        {/* Metric indicators overlay if locked */}
        {!shop.onboarding_complete && (
          <div className="absolute inset-0 z-20 flex items-center justify-center">
            <div className="bg-white/90 backdrop-blur-md px-6 py-3 rounded-xl border border-dove/20 shadow-subtle flex items-center gap-2.5">
              <Lock className="w-4 h-4 text-rust animate-pulse" />
              <span className="text-xs font-semibold text-ink">Overview metrics will unlock when you go live</span>
            </div>
          </div>
        )}
