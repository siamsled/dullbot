...
function RetailBranch({ shop, onNext, onBack }: Props) {
  ...
  const inputCls = 'w-full bg-white/5 border border-white/10 rounded-xl py-2.5 px-3.5 text-white text-sm focus:border-white/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/20 transition-all duration-200 ease-out placeholder:text-white/30 resize-none';
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="bg-white/4 rounded-2xl border border-white/8 p-6 backdrop-blur-md">
          <div className="flex items-center justify-between">
            ...
            <button type="button" role="switch" aria-checked={bulkEnabled} onClick={() => setBulkEnabled(!bulkEnabled)} className={`relative w-11 h-6 rounded-full transition-colors duration-200 ease-out shrink-0 ml-4 active:scale-[0.96] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 ${bulkEnabled ? 'bg-white' : 'bg-white/15'}`}>
              <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-black shadow transition-transform duration-200 ease-out ${bulkEnabled ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>
          {bulkEnabled && (
      ...
      <div className="flex items-center justify-between pt-3 pb-0.5 shrink-0 border-t border-white/8 mt-2">
        ...
        <button onClick={handleSave} disabled={loading} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-white text-black text-sm font-semibold hover:bg-white/90 active:scale-[0.98] disabled:opacity-30 transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50">
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto pb-4 scroll-smooth">
        ...
        <div className="bg-white/4 rounded-2xl border border-white/8 p-6 space-y-4 backdrop-blur-md">
...
function ServiceBranch({ shop, onNext, onBack }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between pt-3 pb-0.5 shrink-0 border-t border-white/8 mt-2">
        <button onClick={onBack} className="inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-white active:scale-[0.98] transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30 rounded-lg px-1"><ArrowLeft className="w-4 h-4" /> Back</button>
        <button onClick={handleContinue} disabled={loading} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-white text-black text-sm font-semibold hover:bg-white/90 active:scale-[0.98] disabled:opacity-30 transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50">
          {loading ? <Loader2 className="w-4 h-4 animate-spin text-black" /> : <>Continue <ArrowRight className="w-4 h-4" /></>}
        </button>
      </div>
    </div>
