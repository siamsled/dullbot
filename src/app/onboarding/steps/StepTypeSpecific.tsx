  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto pb-4">
        ...
        <p className="text-sm text-slate-500 mb-5 leading-relaxed">Let DullBot know how to handle bulk pricing questions from your customers.</p>
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-5">
          <div className="flex items-center justify-between">
            <div><h3 className="text-sm font-semibold text-slate-900">Bulk pricing enabled</h3><p className="text-xs text-slate-400 mt-0.5">AI will reference your pricing policy.</p></div>
            <button type="button" role="switch" aria-checked={bulkEnabled} onClick={() => setBulkEnabled(!bulkEnabled)} className={`relative w-11 h-6 rounded-full transition-colors duration-200 shrink-0 ml-4 ${bulkEnabled ? 'bg-blue-500' : 'bg-slate-200'}`}>
          ...
          {bulkEnabled && (
            <div className="mt-4"><label className="block text-xs font-semibold text-slate-700 mb-1.5">Bulk pricing rules</label>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-100 shrink-0">
        ...
        <button onClick={handleSave} disabled={loading} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-40 transition-colors">
...
function RestaurantBranch({ shop, onNext, onBack }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto pb-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 leading-tight mb-1">A few details specific to your industry.</h1>
        <p className="text-sm text-slate-500 mb-5 leading-relaxed">Add your location so customers can find you and book tables.</p>
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-5 space-y-4">
          <div><label className="block text-sm font-semibold text-slate-700 mb-1.5">Physical Address</label><input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="e.g. 45 Gulshan Ave, Dhaka 1212" className={inputCls} /></div>
          <div><label className="block text-sm font-semibold text-slate-700 mb-1.5">Google Maps Link <span className="text-slate-400 font-normal">(optional)</span></label><input type="url" value={mapLink} onChange={(e) => setMapLink(e.target.value)} placeholder="https://maps.google.com/…" className={inputCls} /></div>
        </div>
      </div>
      <div className="flex items-center justify-between pt-4 border-t border-slate-100 shrink-0">
        <button onClick={onBack} className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-600 transition-colors"><ArrowLeft className="w-4 h-4" /> Back</button>
...
function ServiceBranch({ shop, onNext, onBack }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto pb-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 leading-tight mb-1">A few details specific to your industry.</h1>
        <p className="text-sm text-slate-500 mb-5 leading-relaxed">DullBot handles appointment bookings, service inquiries, and staff scheduling for you.</p>
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-5">
          ...
          <ul className="space-y-2">{['Staff / resource availability management', 'Customer appointment booking via Messenger or WhatsApp', 'Automatic booking confirmations sent to customers', 'Deposit collection support (optional)'].map((point) => (
            <li key={point} className="flex items-start gap-2 text-sm text-slate-500"><span className="w-5 h-5 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold">✓</span>{point}</li>
          ))}</ul>
        </div>
      </div>
      <div className="flex items-center justify-between pt-4 border-t border-slate-100 shrink-0">
        <button onClick={onBack} className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-600 transition-colors"><ArrowLeft className="w-4 h-4" /> Back</button>
        <button onClick={handleContinue} disabled={loading} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-40 transition-colors">
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Continue <ArrowRight className="w-4 h-4" /></>}
        </button>
      </div>
    </div>
