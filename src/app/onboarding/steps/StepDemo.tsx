...
export default function StepDemo({ shop, onBack }: Props) {
  ...
  const handleSend = () => {
    ...
    // Simulate a quick bot echo
    setTimeout(() => {
      setChatMessages((prev) => [...prev, { from: 'bot', text: `Thanks for your message! In your live shop, DullBot will reply using your full product catalog and settings. This is just a preview sandbox.` }]);
    }, 800);
  };
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-slate-500 mb-8 leading-relaxed">
        Here's a live preview. Send a message to see how your AI assistant responds.
      </p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
        {/* Left: Meet your assistant */}
        <div className="bg-slate-50 rounded-xl border border-slate-200 p-5 flex flex-col gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-slate-900">Meet your assistant</p>
              <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-green-600 bg-green-100 px-2 py-0.5 rounded-full">
                <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" /> Live sandbox
              </span>
            </div>
          </div>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-slate-700 text-right max-w-[60%] truncate">{shop.name || 'Your Shop'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Persona</span>
              <span className="font-semibold text-slate-700 capitalize">{shop.tone_template || 'warm'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Channels</span>
              <span className="font-semibold text-slate-700 text-right max-w-[60%]">{channels}</span>
            </div>
          </div>
        </div>

        {/* Right: Chat mockup */}
        <div className="bg-white rounded-xl border border-slate-200 flex flex-col overflow-hidden" style={{ minHeight: '240px' }}>
          <div className="bg-slate-50 border-b border-slate-200 px-4 py-2.5 flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-blue-500 flex items-center justify-center">
              <Sparkles className="w-3 h-3 text-white" />
            </div>
            <span className="text-xs font-semibold text-slate-700">DullBot — {shop.name || 'Your Shop'}</span>
          </div>
          ...
          <div className="flex-1 p-3 space-y-3 overflow-y-auto max-h-48">
            ...
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.from === 'bot' && (
                  <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center mr-1.5 mt-1 shrink-0">
                ...
                {msg.from === 'user' && (
                  <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center ml-1.5 mt-1 shrink-0">
          ...
          <div className="border-t border-slate-200 p-2 flex gap-2">
            <input
              ...
              placeholder={chatMessages.length > 0 ? 'Type another…' : 'Send to test'}
              disabled={demoState === 'loading'}
              className="flex-1 text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500 placeholder:text-slate-400"
            />
            <button
              onClick={handleSend}
              disabled={!chatInput.trim() || demoState === 'loading'}
              className="w-8 h-8 rounded-lg bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 transition-colors disabled:opacity-40"
            >
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={completing}
          className="inline-flex items-center gap-2 px-6 py-3 rounded-full font-semibold text-sm text-white transition-all hover:opacity-90 disabled:opacity-50"
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-slate-500">
                <strong>{shop.name || 'Your shop'}</strong> is now replying to customers 24/7.
              </p>
              <div className="mt-4 flex items-center justify-center gap-1.5 text-xs text-slate-400">
                <Loader2 className="w-3 h-3 animate-spin" /> Taking you to your dashboard…
              </div>
            </motion.div>
          </motion.div>
        )}
