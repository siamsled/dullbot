        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-white/4 rounded-2xl border border-white/8 p-5 flex flex-col gap-4 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-white leading-none">Meet your assistant</p>
                <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded-full mt-1 border border-emerald-500/20 shadow-[0_0_10px_rgba(16,185,129,0.15)]">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Live sandbox
                </span>
              </div>
            </div>
            <div className="space-y-2 text-sm pt-1">
              <div className="flex justify-between items-center"><span className="text-white/40 text-xs">Trained on</span><span className="font-semibold text-white/90 text-xs text-right max-w-[60%] truncate">{shop.name || 'Your Shop'}</span></div>
              <div className="flex justify-between items-center"><span className="text-white/40 text-xs">Persona</span><span className="font-semibold text-white/90 text-xs capitalize">{shop.tone_template || 'warm'}</span></div>
              <div className="flex justify-between items-center"><span className="text-white/40 text-xs">Channels</span><span className="font-semibold text-white/90 text-xs text-right max-w-[60%]">{channels}</span></div>
            </div>
          </div>
          {/* Right: chat mockup */}
          <div className="bg-white/4 rounded-2xl border border-white/8 flex flex-col overflow-hidden shadow-sm" style={{ minHeight: '210px' }}>
            ...
            <div className="flex-1 p-3.5 space-y-2.5 overflow-y-auto">
              ...
              {chatMessages.map((msg, i) => (
                <div key={i} className={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[80%] px-3.5 py-2.5 rounded-2xl text-xs leading-relaxed ${msg.from === 'user' ? 'bg-white text-black rounded-br-sm shadow-sm' : 'bg-white/8 text-white/90 rounded-bl-sm border border-white/8'}`}>{msg.text}</div>
            ...
            <div className="border-t border-white/8 p-2.5 flex gap-2">
              <input ref={inputRef} type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder={chatMessages.length > 0 ? 'Type another…' : 'Send to test'} disabled={demoState === 'loading'} className="flex-1 text-xs bg-white/5 border border-white/10 rounded-xl px-3.5 py-2 text-white focus:outline-none focus:border-white/30 focus-visible:ring-2 focus-visible:ring-white/20 transition-all duration-200 ease-out placeholder:text-white/30" />
              <button onClick={handleSend} disabled={!chatInput.trim() || demoState === 'loading'} className="w-8 h-8 rounded-xl bg-white text-black flex items-center justify-center hover:bg-white/90 active:scale-[0.98] transition-all duration-200 ease-out disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50"><Send className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showDeployModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.94, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[rgba(12,13,18,0.95)] backdrop-blur-2xl border border-white/10 rounded-2xl p-8 max-w-sm w-full text-center flex flex-col items-center gap-4 shadow-2xl">
              <div className="w-16 h-16 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center justify-center shadow-[0_0_20px_rgba(16,185,129,0.2)]"><Check className="w-8 h-8 stroke-[2.5]" /></div>
              ...
              <button onClick={() => window.location.href = '/dashboard'} className="w-full py-3 rounded-xl bg-white text-black text-sm font-semibold hover:bg-white/90 active:scale-[0.98] transition-all duration-200 ease-out mt-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50">Go to Dashboard →</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
