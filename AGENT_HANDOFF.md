...
- **2026-08-03**: Refined `/login` **Halftone Dot-Matrix Liquid Wave** canvas — converted dot lighting into a continuous coherent ocean wave front rolling across the page, replaced harsh displacement impulses with a silky soft Gaussian falloff brush (24-cell radius, cosine falloff) in pure luminous soft white & silver tones (`rgba(245, 248, 255, opacity)`), and tuned viscous elastic damping (`0.95`). Preserved dark glass frosted card (`backdrop-filter: blur(32px)`), 3D hover tilt, and all auth logic. Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
