...
- **2026-08-03**: Built animated `SiriOrb` component (`src/components/ui/siri-orb.tsx`) and integrated it to the left of the `dullbot.` logo in `/onboarding`. Styled `for merchants` as a creative gradient subtext (`from-white/70 via-white/40 to-white/20`) and converted the step counter to a sleek, unconfined tracking layout (`Step 2 / 7` with glowing emerald pulse indicator). Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
