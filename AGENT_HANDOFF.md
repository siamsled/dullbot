...
- **2026-08-03**: Updated `/onboarding` wizard background to match `/login` page — replaced `AuroraBackground` with `PixelLiquidBg` (`pixelSize={14}`, `mouseForce={12}`, `cursorSize={130}`, `autoDemo={false}`). Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
