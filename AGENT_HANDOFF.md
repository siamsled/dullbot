...
- **2026-08-03**: Fixed island card layout alignment in `/onboarding` — updated `GravityStarsBackground` to `pointer-events-none absolute` canvas and adjusted wrapper layout flex centering so the onboarding island card sits dead-center in the viewport. Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
