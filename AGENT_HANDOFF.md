...
- **2026-08-03**: Updated `/onboarding` Step 2 to require at least one connected social channel (**Facebook Messenger**, **Instagram DMs**, or **WhatsApp Business**) before enabling the `Continue` action button. Added strict digits-only regex validation (`^\d{10,20}$`) to WhatsApp Phone Number ID and WABA ID to reject invalid strings containing letters/symbols (e.g. `67rr65r6556`). Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
