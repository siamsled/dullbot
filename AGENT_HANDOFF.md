...
- **2026-08-12**: Executed project-wide performance, caching, security, and UI polish pass: (1) Added `@tanstack/react-query` across Live Inbox, Inventory, Orders, Analytics, Overview; (2) Eliminated N+1 query loop in `getConversations()`; (3) Pre-fetched active conversation messages on server for 0ms load; (4) Added `MessageThreadSkeleton` for conversation switching; (5) Resolved Meta Graph API profile fetching querying `picture.width(200).height(200)` and `profile_pic` fields with fallback to `shop_meta_pages` tokens; (6) Added `getProfilePicUrl` helper in `InboxClient.tsx` rendering `conv.meta_profile_pic` or `profile.profile_pic_url` in sidebar and chat header avatars; (7) Built multi-frame `scrollToBottom` lock preventing image loads from leaving chat stuck in the middle; (8) Redesigned AI Briefing card in customer context sidebar into clean white/fog surface with valid phone number filtering; (9) Implemented smart dual sidebar badges — Urgent Red Pill Badge (`bg-red-500 animate-pulse`) for human takeover/complaint escalations, and Blue Pill Badge for unread customer messages; (10) Built fullscreen blurred media lightbox overlay (`previewMedia`) preventing chat attachments from opening in new tabs; (11) Redesigned dashboard sidebar to slim `w-56` layout, placed `Overview` first, added sleek chevron pin toggle button (`ChevronLeft`/`ChevronRight`), and built unified shop avatar + logout footer card; (12) Updated sidebar `<nav>` to `flex flex-col justify-between` to adaptively span full vertical browser screen height; (13) Implemented `localStorage` state persistence for collapsed sidebar (`dullbot_sidebar_collapsed`) preventing auto-opening on tab switches, paired with hover-expansion (`onMouseEnter`/`onMouseLeave`); (14) Refined all skeleton loaders in `SkeletonLoaders.tsx` and dedicated `loading.tsx` routes to match actual page UI layouts 1-to-1 (`OrdersListSkeleton` with `Lifecycle Control` title, 6 funnel stage cards, search bar, & table layout). Verified clean Next.js build (`npm run build`).
- **2026-08-14**: Standardized Dashboard on original Steep Style light theme. (1) Diagnosed and resolved the split-theme artifact on `/analytics` where the native scroll container revealed a black page wrapper and a white body under it by completely removing the `.dark` Tailwind styling block from `globals.css` (enforcing global light mode for the dashboard). (2) Simplified `UiverseGlassCard.tsx` stat cards by stripping ad-hoc brown and blue-grey gradients and replacing them with native Steep Style background colors (`bg-white`, `bg-apricot-wash`, `bg-sky-wash`). (3) Established consistent metric tinting in `OverviewClient.tsx`—all standard metrics use neutral `variant="default"`, and only "Needs Attention Right Now" cards (pending/aging/flagged/low stock) use the warm `bg-apricot-wash` background and border tinting. All changes compiled and committed.
