...
- **2026-08-03**: Redesigned `/login` visual layer — dark deep-purple ambient background, 3 organic blobs (independent morph `border-radius` + drift `translate` keyframes, 9–13 s loops, `filter: blur(48px)`), cursor spotlight (fixed radial-gradient div, `transform: translate` on `mousemove`, `0.12s linear`), frosted-glass card (`backdrop-filter: blur(24px)` + `-webkit-backdrop-filter` for Safari), 3D card tilt on cursor (`perspective(900px)` wrapper, max 6 deg rotateX/Y, resets on `mouseleave`), fade+scale entrance animation (`card-enter` keyframe). All auth logic (Google OAuth, password sign-in, session cookie slimming) untouched. TypeScript clean (`npx tsc --noEmit`), pushed to main.
