...
- **2026-08-12**: Executed project-wide performance, caching, and security pass: (1) Added `@tanstack/react-query` to wrap client-fetched data across Live Inbox, Inventory, Orders, Analytics, and Overview with 5-min staleTime and zero-reload tab switching; (2) Eliminated N+1 query loop in `getConversations()` in `src/app/dashboard/inbox/actions.ts` by replacing sequential message fetches with a single batched `.in()` query; (3) Conducted API key security audit confirming 0 exposed client secrets; (4) Created reusable glass/fog `SkeletonLoaders.tsx` for Live Inbox, Inventory, Orders, Analytics, and Overview; (5) Redesigned onboarding top-right glass action pill to cleanly present step counter and user account switch button with gradient initial avatar dot; (6) Pre-fetched initial conversation messages server-side in `/dashboard/inbox/page.tsx` for 0ms active thread load time; (7) Integrated `MessageThreadSkeleton` for seamless thread switching; (8) Added `getDisplayName` helper preventing 20-digit raw Meta PSIDs from displaying as customer names; (9) Added `formatSnippetPreview` stripping raw markdown image URLs `![alt](url)` into clean `📷 Photo` previews; (10) Verified zero TypeScript errors (`npx tsc --noEmit`) and successful production build (`npm run build`). Committed & pushed `10cb7a2`.
