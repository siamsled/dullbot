...
- **2026-08-03**: Redesigned `/login` visual layer — strictly cool analogous palette (violet `#8b5cf6`, indigo `#4f46e5`, soft cyan/teal `#0ea5e9`), 3 organic blobs with independent morph & drift CSS keyframes (8-14s loops, non-cursor reactive). Added mousemove particle trail (throttled to 60ms, 10-20px radial gradient particles in cool palette, random upward drift & fade over 1.6s, automatic React state DOM cleanup timeout). Preserved frosted-glass card (`backdrop-filter` + `-webkit-backdrop-filter`), 3D card tilt on hover (max 6deg rotateX/Y), and fade+scale entrance animation. Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
