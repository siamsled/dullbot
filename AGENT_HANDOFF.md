...
- **2026-08-03**: Redesigned onboarding header with `dullbot.` serif logo (matching `/login` card), replaced generic category icons with local BD business icons (`Store`, `Utensils`, `Scissors`), removed Back button from Step 1 footer, and reduced background cursor displacement force (`mouseForce={3.5}`, `cursorSize={75}`) across `/login` and `/onboarding` for smooth, eye-friendly fluid warping. Verified clean build (`npx tsc --noEmit`) and pushed to main.
