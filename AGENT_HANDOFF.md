...
- **2026-08-03**: Replaced generic icons with official brand SVG logos for **Facebook Messenger** (`#0084FF`), **Instagram** (`#E4405F`), and **WhatsApp** (`#25D366`) in `/onboarding` Step 2. Enlarged header logo and `SiriOrb` size (36px), removed `Step` text and green pulse dot for a clean step counter (`1 / 7`), and integrated `GravityStarsBackground` (`src/components/ui/gravity-stars-bg.tsx`). Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
