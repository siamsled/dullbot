...
- **2026-08-03**: Redesigned `/login` visual layer with a Windows 11 3D Bloom-inspired theme — vibrant Electric Cyan (`#00b4d8`), Fluent Purple (`#9d4edd`), and Hot Magenta (`#e056fd`) morphing blobs using `mix-blend-mode: screen`, desktop micro-grid background, Windows 11 Acrylic frosted card (`backdrop-filter: blur(32px)`), glow-on-focus input states (`#00b4d8` glow), cursor particle trail, and 3D card tilt. Preserved all auth logic (Google OAuth, password sign-in, session slimming). Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
