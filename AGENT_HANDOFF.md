...
- **2026-08-03**: Fixed viewport centering for `/onboarding` island card — separated `GravityStarsBackground` canvas into a background layer and wrapped the card in a `fixed inset-0 flex items-center justify-center` stage container so the card is locked dead-center in the screen regardless of canvas dimensions. Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
