...
- **2026-08-03**: Redesigned `/login` background to match the **Halftone Dot-Matrix Liquid Wave** artwork reference — high-density dot grid canvas with doubled dot population density (`GRID_GAP = 7px`, tiny high-res dots), luminous S-curve wave field luminance, and dynamic 2D displacement grid (`gridDisp`) that physically warps the dots and wave paths in real time as the cursor moves across the screen. Styled with a dark glass frosted card (`backdrop-filter: blur(32px)`), crisp contrast typography, and 3D hover tilt. Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
