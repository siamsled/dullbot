...
- **2026-08-03**: Added strict input validation to WhatsApp Cloud API modal (verifies 10-20 digit numeric WABA ID / Phone Number ID and valid token length before saving), and added `Disconnect` action buttons to connected channels in `/onboarding` Step 2 with server actions (`disconnectFacebook`, `disconnectInstagram`, `disconnectWhatsApp`). Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
