...
- **2026-08-03**: Fixed viewport centering for `/login` page card — separated `GravityStarsBackground` into an absolute background layer and wrapped the login card stage in a `fixed inset-0 flex items-center justify-center` container so the card is locked dead-center in the viewport on all screen resolutions. Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
