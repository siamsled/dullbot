...
- **2026-08-03**: Integrated exact `PixelLiquidBg` component for `/login` background — Navier-Stokes 2D fluid simulation built with Three.js WebGL shaders, Bayer 4x4 matrix dithering, custom palette color mapping, film-grain noise, and auto-demo mode yielding to cursor interaction. Installed `three` and `@types/three`. Preserved dark frosted glass card (`backdrop-filter: blur(32px)`), 3D hover tilt, and all auth logic. Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
