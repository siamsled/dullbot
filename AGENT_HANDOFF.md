...
- **2026-08-03**: Updated `/login` background & card layout — disabled `autoDemo` (`autoDemo={false}`) so fluid motion only triggers on actual user cursor movement and decays through natural Navier-Stokes momentum physics when cursor stops. Dead-centered login glass card horizontally and vertically across all viewport sizes (`mx-auto my-auto flex items-center justify-center`). Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
