...
- **2026-08-12**: Executed project-wide performance, caching, security, and UI polish pass: (1) Added `@tanstack/react-query` across Live Inbox, Inventory, Orders, Analytics, Overview; (2) Eliminated N+1 query loop in `getConversations()`; (3) Pre-fetched active conversation messages on server for 0ms load; (4) Added `MessageThreadSkeleton` for conversation switching; (5) Resolved Meta Graph API profile fetching (`fields=name,first_name,last_name,profile_pic`) with `shop_meta_pages` token lookup fallback; (6) Built multi-frame `scrollToBottom` lock preventing image loads from leaving chat stuck in the middle; (7) Redesigned AI Briefing card in customer context sidebar into clean white/fog surface with valid phone number filtering; (8) Implemented smart dual sidebar badges — Urgent Red Pill Badge (`bg-red-500 animate-pulse`) for human takeover/complaint escalations, and Blue Pill Badge for unread customer messages; (9) Removed big redundant Inbox title header; (10) Redesigned double filter bars into compact, proportional segmented pill controls; (11) Enhanced `getDisplayName` order/facts name fallbacks; (12) Added explicit `router.push()` to `Sidebar.tsx` nav links fixing tab switching in Next.js App Router. Verified clean Next.js build (`npm run build`) and pushed `51f83ea` to `main`.
