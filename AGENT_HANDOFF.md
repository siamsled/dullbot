...
- **2026-08-03**: Synchronized `GravityStarsBackground` across `/login` and `/onboarding` pages. Added global window `mousemove` and `touchmove` listeners to `GravityStarsBackground` (`src/components/ui/gravity-stars-bg.tsx`) so cursor attraction and repulsion work smoothly across the entire screen even when hovering over card overlays. Fixed layout centering for onboarding island card (`max-w-3xl` flex centered). Verified zero TypeScript errors (`npx tsc --noEmit`) and pushed to main.
