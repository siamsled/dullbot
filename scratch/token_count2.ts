...
const personas = [
  ...
  },
  {
    ...
    msg_discount_decline: 'ভাই, দাম তো ফিক্সড। এটা নিয়ে আর কিছু করা যাচ্ছে মহাশয় না আসলে।',
...
async function run() {
  for (const p of personas) {
    ...
    // The current prompt-builder outputs English rules mixed with the Bengali phrases
    const newPrompt = buildSystemPrompt(mockShop as any, p as any, [], []);
    
    const countOld = await model.countTokens(oldPrompt);
    const countNew = await model.countTokens(newPrompt);
